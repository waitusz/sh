		...
		1)
			...
			while true; do
				...
				if ss -tuln | grep -q ":$app_port "; then
					echo -e "${gl_hong}错误: ${gl_bai}連接埠$app_port 已被占用，请更换一个端口"
					send_stats "应用端口已被占用"
				else
...
SYSCTL
	...
	echo -e "${gl_lv}已應用${applied} 项参数${skipped:+，跳过 ${skipped} 项不支持的参数}${gl_bai}"
...
LIMITS
	...
	echo -e "${gl_lv}内存: ${MEM_MB}MB | 拥塞算法: ${CC}| 隊列:${QDISC}${gl_bai}"
}
...
add_connection() {
	...
	kj_ssh_read_host_user_port "请输入IP地址: " "请输入用户名 (默认: root): " "請輸入連接埠號碼 (預設: 22):" "root" "22"
...
sync-openclaw-provider-interactive() {
	...
	if [ -z "$provider_name" ]; then
		if sync_openclaw_api_models; then
			...
			echo "❌ API 模型同步失败，已中止重启网关。請檢查 provider /models 返回後重試。"
			return 1
		fi
		break_end
		return 0
...
for entry in data:
    ...
    if isinstance(vec, dict) and vec.get("enabled"):
        vec_str = "就緒" if vec.get("available") else "已启用(不可用)"
    else:
        vec_str = "未启用"
    print("  向量库: %s" % vec_str)
...
log_menu() {
	...
	while true; do
		...
		case $choice in
			1)
				send_stats "查看最近日誌"
				read -erp "查看最近多少行日志？ [預設 100]:" lines
				lines=${lines:-100}
